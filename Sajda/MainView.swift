...
struct MainView: View {
    ...
    var body: some View {
        // Pengecekan untuk loading state
        if vm.todayTimes.isEmpty && vm.authorizationStatus == .authorized {
            VStack(spacing: 10) {
                ProgressView()
                Text(vm.timeZoneIdentifier.isEmpty ? "Calculating..." : vm.timeZoneIdentifier)
                    .foregroundColor(.secondary)
            }
            .frame(width: 260, height: 200)
        } else {
            // Tampilan utama setelah jadwal sholat didapatkan
            ...
            .padding(.vertical, 8).frame(width: 260)
        }
    }
}
